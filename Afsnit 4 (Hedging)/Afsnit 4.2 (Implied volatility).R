...
) %>% layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz, aspectratio = list(x=1, y=1, z=0.6))) %>% colorbar(title = "Implied volatility") %>% layout(
  scene= list(
    camera= list(
...
) %>% layout(xaxis = axx, yaxis = axy, autosize = F, width = 600, height = 450, margin = m) %>% colorbar(title = "Implied volatility") 
